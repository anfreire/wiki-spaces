...
- **Parent's tier matters for the `## Spaces` update step.** If the parent `index.md` has a `## Spaces` section, the mount must be listed there (the navigability contract). If the parent has no `## Spaces`, you have two choices: (a) add `## Spaces` to the parent's `index.md` now, upgrading it to Tier 2 and then listing the mount; or (b) leave the parent at Tier 1 and skip the listing — the mount exists on disk but isn't navigable from the parent's index. Ask the user which they want.
- **Shortcut for the `## Spaces` update.** After the filesystem step in any branch below, run `wiki-spaces space add <relative-path>` to register the mount with the nearest ancestor. It's idempotent on an existing `index.md` (won't overwrite the mounted content) and walks up to the right ancestor space automatically. **The CLI atomically refuses** when the nearest ancestor is Tier 1 (no `## Spaces`): it exits non-zero and touches nothing. Resolution: add `## Spaces` to the ancestor's `index.md` first (a single heading line), then rerun.
