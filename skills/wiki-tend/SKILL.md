...
5. **Audit (report only).** Run `wiki-spaces space audit` (add `--wiki <path>` when the scope is a named sub-space, not the canonical wiki) for the structural facts. It walks owned scope and reports three things:
   ...
   The CLI is the source of truth for those three — don't re-derive them by hand. Then add the two judgment-bearing checks it does not do:
   ...
   Orphans and broken links are *facts*; deciding whether an orphan is acceptable, or how to repair a broken link, is judgment — apply it when presenting the report.
