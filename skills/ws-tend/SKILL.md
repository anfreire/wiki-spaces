...
5. **Audit.** Run `audit` and relay its findings in its own vocabulary — `contract` (a bare index and whether it is registered, a near-miss heading, a malformed / duplicate / unregistrable entry, an entry crossing another space's boundary, a dead second heading), `drift` (an on-disk space not listed / a listed entry with nothing on disk), `mount` (a registered external mount that is no longer a wiki), `broken` links, `over-cap` files, `unreadable` files, and informational `orphans`. The script is the source of truth for those; don't re-derive them. Add the judgment it can't make:
