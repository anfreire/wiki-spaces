...
3. Build the groups — default mode is by tag (top 10 by usage, counted from a `ws.py grep '^tags:'` sweep, when frontmatter is in use); fall back to top-level folders when there's no frontmatter. One group per tag/folder:
   ```json
