...
`topic.md` becomes `topic/index.md`, gaining the right to hold children. Routine once a page's shape asks for it — and since other pages may link to the file that moves, the procedure is inventory, snapshot, move, rewrite, verify, stopping at the first surprise.
...
1. **Inventory every incoming link before moving anything:**
   ```sh
...
2. **Snapshot.** Git wiki: commit the current state (`git -C <root> add -A && git -C <root> commit -m "snapshot: before promoting <page>"`) — or, if the user minds the commit, `git stash push --include-untracked` and note the stash; a worklist file the wiki's git does not track (inside a linked space, say) goes in a tar as below. No git: archive the files this operation touches, paths kept — the page, each worklist file, and the ancestor `index.md` step 6 edits: `tar -C <root> -cf <scratch>/pre-promote.tar <rel>…`. Do not proceed without a restore path.
3. **Move:**
   ```sh
...
7. **Verify:** `python3 <skill-dir>/scripts/ws.py audit --wiki <root>` must report no drift and the new space registered; then re-run step 1's sweep — every worklist occurrence must now speak the new form (`[[<stem>/index|…]]`, `(…/<stem>/index.md)`), and a hit still carrying the old bare form outside a code example is a miss to fix. Then `check-size` the new `index.md`.
8. **On any surprise, restore the snapshot** (`git reset --hard` to the snapshot commit / `git stash pop` / `tar -C <root> -xf <scratch>/pre-promote.tar`), remove what the operation created — `<stem>/` and anything under it — report what blocked, and stop. A half-promoted page is worse than an over-cap one.
